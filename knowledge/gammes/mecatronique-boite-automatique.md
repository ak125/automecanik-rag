...
conseil_v5:
  ...
  S1: 'Piloter electroniquement les passages de vitesses. Pièces liées : vérifier les composants adjacents lors du remplacement.'
  S2: 'Intervalle : selon constructeur. Symptômes de défaillance : - Passages de rapports brutaux- Boite en mode degrade-
    Voyant boite de vitesses allume'
  S3: 'Pour choisir le bon mécatronique boîte automatique pour votre véhicule : - Renseignez marque, modele, type puis comparez
    references et dimensions. Validez ensuite les contraintes de moteur pour confirmer Mécatronique boîte automatique.- Verifier
    la reference OE ou equivalence constructeur pour le vehicule exact- Respecter le type de boite (manuelle, automatique,
    robotisee) et sa generation- Controler la compatibilite des cannelures et dimensions avec le vehicule- Marques : SKF,
    GKN/Spidan (premium), Febi, Meyle (standard), Ridex, Topran (budget)- Budget : 1000 à 5000 EUR'
  ...
  S5: 'Erreurs fréquentes avec le mécatronique boîte automatique : - Ne pas vérifier la référence exacte avant commande —
  ...
  S6: 'Après le remplacement du mécatronique boîte automatique : - Verifier le niveau d huile de boite selon preconisation
    constructeur- Controle des soufflets de protection (pas de fuite de graisse)- Remplacement de la bague d etancheite en
    cas de fuite- Inspection des cardans et croisillons a chaque revision- Effacer les codes défaut éventuels avec l''outil
